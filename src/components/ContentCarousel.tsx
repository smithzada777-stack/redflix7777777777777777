...
export default function ContentCarousel({ title, items, delay = 0 }: CarouselProps) {
    ...
    return (
        <section className="py-2 md:py-8 relative w-full overflow-hidden">
            {/* Subtitle */}
            <h4 className="container mx-auto px-6 md:px-12 text-[17px] md:text-[23px] font-bold mb-4 md:mb-6 text-white uppercase tracking-widest italic">
                {title}
            </h4>

            <div
                className="relative w-full overflow-hidden group pt-4 pb-2 md:pb-10"
                onMouseEnter={() => setIsPaused(true)}
                onMouseLeave={() => setIsPaused(false)}
            >
                {/* Lateral Blurs */}
                <div className="absolute left-0 top-0 bottom-0 w-12 md:w-48 bg-gradient-to-r from-black via-black/80 to-transparent z-20 pointer-events-none" />
                <div className="absolute right-0 top-0 bottom-0 w-12 md:w-48 bg-gradient-to-l from-black via-black/80 to-transparent z-20 pointer-events-none" />
